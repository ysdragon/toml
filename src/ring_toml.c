...
static List *toml_table_to_ring_list(VM *pVM, toml_datum_t table_datum)
{
	if (table_datum.type != TOML_TABLE)
		return NULL;
...
static List *toml_array_to_ring_list(VM *pVM, toml_datum_t array_datum)
{
	if (array_datum.type != TOML_ARRAY)
		return NULL;
...
static void add_converted_toml_value(VM *pVM, List *pList, toml_datum_t datum)
{
	...
	case TOML_BOOLEAN:
		...
		break;
	case TOML_DATE:
		snprintf(buffer, sizeof(buffer), "%04d-%02d-%02d", datum.u.ts.year, datum.u.ts.month, datum.u.ts.day);
		ring_list_addstring_gc(pVM->pRingState, pList, buffer);
		break;
	case TOML_TIME:
		if (datum.u.ts.usec > 0)
			snprintf(buffer, sizeof(buffer), "%02d:%02d:%02d.%06d", datum.u.ts.hour, datum.u.ts.minute,
					 datum.u.ts.second, datum.u.ts.usec);
		else
			snprintf(buffer, sizeof(buffer), "%02d:%02d:%02d", datum.u.ts.hour, datum.u.ts.minute, datum.u.ts.second);
		ring_list_addstring_gc(pVM->pRingState, pList, buffer);
		break;
	case TOML_DATETIME:
		if (datum.u.ts.usec > 0)
			snprintf(buffer, sizeof(buffer), "%04d-%02d-%02dT%02d:%02d:%02d.%06d", datum.u.ts.year, datum.u.ts.month,
					 datum.u.ts.day, datum.u.ts.hour, datum.u.ts.minute, datum.u.ts.second, datum.u.ts.usec);
		else
			snprintf(buffer, sizeof(buffer), "%04d-%02d-%02dT%02d:%02d:%02d", datum.u.ts.year, datum.u.ts.month,
					 datum.u.ts.day, datum.u.ts.hour, datum.u.ts.minute, datum.u.ts.second);
		ring_list_addstring_gc(pVM->pRingState, pList, buffer);
		break;
	case TOML_DATETIMETZ: {
		...
		if (datum.u.ts.usec > 0)
			snprintf(buffer, sizeof(buffer), "%04d-%02d-%02dT%02d:%02d:%02d.%06d%c%02d:%02d", datum.u.ts.year,
					 datum.u.ts.month, datum.u.ts.day, datum.u.ts.hour, datum.u.ts.minute, datum.u.ts.second,
					 datum.u.ts.usec, sign, tz_hour, tz_min);
		else
			snprintf(buffer, sizeof(buffer), "%04d-%02d-%02dT%02d:%02d:%02d%c%02d:%02d", datum.u.ts.year,
					 datum.u.ts.month, datum.u.ts.day, datum.u.ts.hour, datum.u.ts.minute, datum.u.ts.second, sign,
					 tz_hour, tz_min);
		ring_list_addstring_gc(pVM->pRingState, pList, buffer);
	}
	break;
	...
	case TOML_TABLE:
		pSourceList =
			(datum.type == TOML_ARRAY) ? toml_array_to_ring_list(pVM, datum) : toml_table_to_ring_list(pVM, datum);
		if (pSourceList)
		{
...
RING_FUNC(ring_toml_parse)
{
	...
	if (!result.ok)
	{
		...
		gc_toml_last_error[sizeof(gc_toml_last_error) - 1] = '\0';
		toml_free(result);
		return;
	}
...
RING_FUNC(ring_toml_get_ex)
{
	...
	if (datum.type == TOML_ARRAY || datum.type == TOML_TABLE)
	{
		List *pList = (datum.type == TOML_ARRAY) ? toml_array_to_ring_list((VM *)pPointer, datum)
												 : toml_table_to_ring_list((VM *)pPointer, datum);
		if (pList)
		{
	...
	/* For simple types (string, number, date, etc.), use the wrapper pattern */
